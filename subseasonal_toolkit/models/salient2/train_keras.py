...
Example:
        $ python src/models/salient2/train_keras.py d2wk_cop_sst_20170201 -n 50 -s 0 
        $ python src/models/salient2/train_keras.py d2wk_cop_sst_mei_mjo_20190731 -n 40 -s 0 
        $ python src/models/salient2/train_keras.py d2wk_cop_sst_mei_mjo_20190731 -n 10 -s 40
        $ python src/models/salient2/train_keras.py d2wk_cop_sst_mei_20190731 -n 50 -s 0 -r contest 
        $ python src/models/salient2/train_keras.py d2wk_cop_sst_mei_20190731 -n 50 -s 0 -r us
...
from subseasonal_toolkit.models.salient2.salient2_util import dir_train_data, dir_train_results, val_i_error
...
def compile_input(submodel_name, n_weeks_predict=[3,4,5,6], i_time=False, region='contest'):
    ...
    Args:
        ...
        region (str):  string consisting of spatial region used for output data, 
                        either 'contest' or 'us' (default: 'contest').
    ...
    #get gt_var and end_date and dir_train_data:
    ...
    location_precip_file = os.path.join(dir_train_data, f"{region}_precip.pickle")
    precip_data = pickle.load(open(location_precip_file, 'rb'))
    precip_data = precip_data.T

    # load temperature data
    location_temp_file = os.path.join(dir_train_data, f"{region}_tmp2m.pickle")
    ...
    # Ensure same length vectors and standardize mei data    
    if i_mei:
        mei_vectors = mei_vectors[:precip_data.shape[0],:]
        mei_vectors = (mei_vectors - data_min) * 1./(data_max - data_min)

    # Ensure same length vectors and standardize mjo data    
    if i_mjo:
        mjo_vectors = mjo_vectors[:precip_data.shape[0],:]
    ...
    if i_mei:
        mei_vectors = mei_vectors[:-max_weeks_predict, ...] 
    if i_mjo:
        mjo_vectors = mjo_vectors[:-max_weeks_predict, ...]     
...
def main():
    ...
    for dir_name in ["logs", "checkpoints", "histories", "histories_detailed"]:
        make_directories(os.path.join(in_dir_weights, dir_name))
    ...
    for i in range(n_random_models):
        ...
        epochs = 1#random.randint(100,500)
