...
def get_submodel_name(forecast="cfsv2", fit_intercept=True, train_years="all", margin_in_days=None,
                      first_day=1, last_day=1, loss="mse", first_lead=0, last_lead = 29, 
                      mei=False, mjo=False):
    """Returns submodel name for a given setting of model parameters
    """
    submodel_name = f"{forecast}pp-debias{fit_intercept}_years{train_years}_margin{margin_in_days}_days{first_day}-{last_day}_leads{first_lead}-{last_lead}_loss{loss}"
