...
def get_submodel_name(num_steps=3, target_year=2018):
    """Returns submodel name for a given setting of model parameters
    """
    if target_year == 'all':
        submodel_name = f"{MODEL_NAME}_steps_{num_steps}"
    else:
        submodel_name = f"{MODEL_NAME}_steps_{num_steps}_year_{target_year}"
